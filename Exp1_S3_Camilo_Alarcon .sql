...
/* -------- CASO 1  ----------------------------------*/
ACCEPT RENTA_MINIMA PROMPT 'Ingrese renta mínima: ';
ACCEPT RENTA_MAXIMA PROMPT 'Ingrese renta máxima: ';

SELECT 
    REGEXP_REPLACE(TO_CHAR(numrut_cli), '(\d{1,3})(\d{3})(\d{3})', '\1.\2.\3') 
    ...
    INITCAP(celular_cli) AS "Celular Cliente",
...
/* -------- CASO 2  ----------------------------------*/
ACCEPT SUELDO_PROMEDIO_MINIMO PROMPT 'Ingrese sueldo promedio mínimo: ';
...
SELECT
    ...
    TO_CHAR(ROUND(AVG(e.sueldo_emp)), 'L999G999G999') AS SUELDO_PROMEDIO
FROM empleado e
JOIN categoria_empleado c ON e.id_categoria_emp = c.id_categoria_emp
JOIN sucursal s ON e.id_sucursal = s.id_sucursal
GROUP BY c.desc_categoria_emp, s.desc_sucursal
...
SELECT
    ...
    INITCAP(tp.desc_tipo_propiedad) AS DESCRIPCION_TIPO,
    COUNT(p.nro_propiedad) AS TOTAL_PROPIEDADES,
    TO_CHAR(ROUND(AVG(p.valor_arriendo)), 'L999G999G999') AS PROMEDIO_ARRIENDO,
    ROUND(AVG(p.superficie), 2) AS PROMEDIO_SUPERFICIE,
    ROUND(AVG(p.valor_arriendo / p.superficie), 0) AS VALOR_ARRIENDO_M2,
    CASE 
